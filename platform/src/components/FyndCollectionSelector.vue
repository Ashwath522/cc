<template>
  <div class="fynd-selector-overlay" @click.self="$emit('close')">
    <div class="fynd-selector-modal" role="dialog" aria-labelledby="collection-selector-title">
      <header class="fynd-selector-header">
        <div>
          <p class="fynd-selector-eyebrow">Browse</p>
          <h2 id="collection-selector-title" class="fynd-selector-title">Select collection</h2>
          <p class="fynd-selector-sub">Search by name or slug, then pick a collection to link.</p>
        </div>
      ...
      <div class="fynd-selector-body">
        ...
        <div v-else-if="items.length === 0" class="fynd-selector-state fynd-selector-state--empty">
          <p class="fynd-selector-empty-title">No collections found</p>
          ...
          <li v-for="item in items" :key="item._id || item.uid || item.slug" class="fynd-selector-row">
            <div class="fynd-selector-row-main">
              ...
              <span class="fynd-selector-meta">
                ...
                <span v-if="item.code" class="fynd-selector-code">Code: {{ item.code }}</span>
              </span>
            </div>
...
<script>
import { searchCollections } from '../services/fyndCollection.service';

export default {
  name: 'FyndCollectionSelector',
  data() {
    return {
      query: '',
      items: [],
  ...
  methods: {
    ...
    async search() {
      ...
      this.loading = true;
      try {
        const response = await searchCollections(this.query.trim());
