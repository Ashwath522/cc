 ...
 * template swap; highlights are assembled only from facts already
...
function buildQualityPromise(category, warranty, specifications) {
  const cat = category ? category.toLowerCase() : 'home';
  const statement = `Every ${cat} product is checked for finish, fit, and everyday usability before it reaches your home.`;
