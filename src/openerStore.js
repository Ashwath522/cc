 ...
 *
 * Key improvements:
 * - structuralPatternCheck: detects narrative template repetition beyond phrase-level
 * - buildDiversityHint: produces a prompt-injectable string listing recent patterns to avoid
 * - source tagging: mock records tagged 'mock' are excluded from all diversity checks
 * - checkPhraseFrequency: skips mock-polluted high-frequency entries for real sentences
 */
const fs = require('fs');
...
// Structural patterns catch variable-substitution templates that phrase-frequency misses
...
function getRecentlyUsedPatterns(n, filePath) {
  const recent = loadRecentOpeners(n, filePath);
